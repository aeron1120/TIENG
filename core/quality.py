...
이관을 시도했다가 되돌린 기록. 합성 신호로 두 공식을 비교해 보면(게이트 0.4):

    조건            구 score()   SQI x 모달리티
    정상              ~1.00        0.77~0.93
    어두움             0.95        0.38~0.47   <- 55/72bpm 이 보류로 넘어간다
    피부 절반(0.175)    0.92        0.38~0.47   <- 정상적인 얼굴인데 보류

함정이 둘이다.
  - 모달리티 성분을 곱으로 넣으면 과하다. 구 공식에서 q_roi 는 가중치 0.15 짜리
    가산항이라 거칠어도 됐는데, 곱으로 바꾸면 정상적인 ROI 가 confidence 를
    절반으로 깎는다. skin_ratio 는 기울기가 아니라 게이트로 다뤄야 한다.
  - brightness 는 이중 계산이다. 실제로 어두우면 SNR 이 붕괴해 SQI 가 이미
    떨어진다. 깨끗한 합성 신호에 밝기 숫자만 낮게 붙이는 것은 현실적인 입력이
    아니라서, 이 비교표의 "어두움" 행도 그 점을 감안해 읽어야 한다.

그래서 계수를 눈대중으로 정하지 않고 재측정 뒤로 미뤘다. 목표 오차에서 임계값을
역산하는 경로가 legacy 쪽에 이미 있다 (Calibration.threshold_for_mae).
